...
## Переход к ранее зафиксированному какому-либо файлу или папки

Для того чтобы перейти к ранее созданному изменению файла или папки необходимо ввести команду:

    git checkout <branch name>

где \<branch name\> в данном случае название папки (ветки).
...
## Удаление выбранной папки (ветки)

Для того чтобы удалить выбранную папку (ветку) нужно ввести команду:

    git branch -d <branch name>

где \<branch name\> имя выбранной папки (ветки).

## Принудительное удаление выбранной ветки

Если возникла необходимость удаления ветки с не зафиксированными изменениями и коммитами, необходимо ввести команду:

    git branch -D <branch name>

где \<branch name\> имя удаляемой ветки.
...
## Merge конфликт и пути решения

При слиянии двух веток проекта в программе Git возможны возникновения конфликтов. Проблемма возникает при расположении текстовой информации в ветках на одинаковых строках. При возникновении merge конфликта необходимо установить причину конфликта текущих (current) данных с входящими (incoming) извне. Для более наглядного сравнения имеющихся отличий можно воспользоваться функцией Compare Changes (Сравнить изменения).
После установления причины, окончательное решение конфликта принимается из 3-х предложенных вариантов: 

1. Accept Current Change-Принять текущее изменение.
2. Accept Incoming Change-Принять входящие изменения.
3. Accept Both Changes-Примите оба изменения.
